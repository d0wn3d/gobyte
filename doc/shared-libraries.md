...
`gobyteconsensus_version` returns an `unsigned int` with the API version _(currently at an experimental `0`)_.
...
- `gobyteconsensus_error* err` - Will have the error/success code for the operation _(see below)_.
...
- `gobyteconsensus_ERR_OK` - No errors with input parameters _(see the return value of `gobyteconsensus_verify_script` for the verification status)_
