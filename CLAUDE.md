# GoByte Core Development Guide

## Overview

GoByte Core is the reference implementation for GoByte, a cryptocurrency. It builds on top of Dash Core, a codebase that
is primarily written in C++. GoByte Core uses the GNU Autotools build system. GoByte Core uses Neoscrypt Proof-of-Work (PoW) algorithm compared to Dash's X11.

## Directory Structure

- **Implementation** `src/` - C++ codebase
  ...
  - `src/qt/` - Implementation of GoByte Qt, the GUI (uses Qt 5)
...
- **Unit Tests**
  - `src/test/`, `src/wallet/test/` - C++ unit tests (uses `Boost::Test`)
  - `src/qt/test/` - C++ unit tests for GUI implementation (uses Qt 5)
- **Functional Tests**: `test/functional/` - Python tests (minimum version in `.python-version`) dependent on `gobyted` and `gobyte-node`
...
- Vendored dependencies:
  - `src/{crc32c,gobytebls,gsl,immer,leveldb,minisketch,secp256k1,univalue}`
  - `src/crypto/{ctaes,neoscrypt}`
...
./src/test/test_gobyte --run_test=getarg_tests

# Debug unit tests
gdb ./src/test/test_gobyte
```
...
GoByte Core is a fork of Dash Core which extends Bitcoin Core through composition, using a layered architecture:

```
GoByte Core Components
├── Bitcoin/Dash Core Foundation (Blockchain, consensus, networking)
...
- **CFlatDB**: A GoByte-specific flat file database format used for persistent storage
...
- **CDBWrapper**: Bitcoin Core database wrapper extended for GoByte-specific data
...
# Run functional test with custom gobyted
test/functional/test_runner.py --gobyted=/path/to/gobyted
...
# Memory debugging
valgrind --leak-check=full ./src/gobyted
```
...
gh api repos/D0WN3D/gobyte/actions/jobs/<JOB_ID>/logs
...
# gh api repos/gobytecoin/gobyte/actions/jobs/46274126203/logs
```
...
- GoByte uses `unordered_lru_cache` for efficient caching with LRU eviction
- The codebase extensively uses GoByte-specific data structures for performance
